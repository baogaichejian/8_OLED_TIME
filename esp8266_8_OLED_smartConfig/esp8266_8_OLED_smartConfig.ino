...
//OLED矩阵时钟
//关注"爆改车间"微信公众号获取更多代码
//该代码为一键配网代码的，开机后按住s3键三秒第一块OLED显示等待配网后使用，
//s1键更换字体风格。
//"乐鑫信息科技"微信公众号中-商铺-Airkiss设备-进行配网（或使用EspTouch APP配网）。
//如果多次配网失败，尝试更换手机或路由器，或直接使用非一件配网代码直接在代码中输入Wi-Fi信息即可。
...
void gettime(){                  //定时器定时器获取网络时间
    ...
    if(WiFi.status() == WL_CONNECTED and tm->tm_year != 70){
      if(Clock.getYear() !=tm->tm_year)Clock.setYear(tm->tm_year);
